...
class Gateway(SocketHelper):
	def __init__(self, name):
		super(Gateway, self).__init__(name)
		self.servers = {} # server_name : {'name', 'host', 'port', 'heartbeat', 'max_user', 'total_user', 'last_heartbeat', 'hold_token', 'connect_time'}
		...
		self.heartbeat_timeout = int(config.get('Server', 'heartbeat_timeout'))/1000
		self.user_lists = {} # username : server_name
		
		self.db_path = 'db/gateway.sqlite'
		conn = sqlite3.connect(self.db_path)
		with open ('db/gateway.sql') as f:
			sql = f.read()
			cur = conn.cursor()
		...
		self.token_check_time = int(config.get('Resource', 'token_hold_time'))*3
		
		self.set_variable('last_token_check', str(int(time.time())))
